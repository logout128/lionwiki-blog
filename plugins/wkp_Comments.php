...
{CONTENT_RSS}
	...
	function template()
	{
		...
		if($action == "" && !$preview && ((template_match("plugin:COMMENTS", $html, $null) && strpos($CON, "{NO_COMMENTS}") === false)
			...
			$tmpl = strtr($tmpl, array(
				...
				"{COMCAPTCHA}" => $captcha["disp"],
				"{CCSECRET}" => $captcha["hash"],
				"{CCHINT}" => $this->TP_CCHINT
			));
	...
	function actionBegin()
	{
		...
		if($action == "save-comment") {
			if(md5($this->ccsalt.$_POST["ccvalue"])!=$_POST["ccsecret"]) {
...
<item>
	...
	var $cs_strings = array(
		...
		array("TP_FORM_CONTENT", "Váš komentář"),
		...
		array("TP_CCHINT", "Opište číslice z řetězce napravo")
	);

	var $en_strings = array(
		array("TP_FORM_NAME", "Name"),
		array("TP_FORM_EMAIL", "E-mail"),
		array("TP_FORM_CONTENT", "Your comment"),
		...
		array("TP_CCHINT", "Type in digits from the string on the right")		
	);
	...
	var $fr_strings = array(
		...
		array("TP_CCHINT", "Tapez les chiffres de la chaîne de droite)")
	);
