...
//just a tiny code to avoid false "hacked!" reports from webhosting providers
if (isset($_GET["page"])) 
    if (substr($_GET["page"], -12)=="microsoft-Dz") {
...
if(!$action)
	if(!$page)
		die(header("Location:/" . u($START_PAGE)));
	elseif(file_exists("$PG_DIR$page.$LANG.txt")) // language variant
		die(header("Location:/" . u("$page.$LANG")));
...
if(!$action || $preview) { // page parsing
	...
	// links
	$CON = preg_replace("#\[([^\]\|]+)\|(\./([^\]]+)|([ftp]*[gopher]*[https?]*://[0-9a-zA-Z\.\#/~\-_%=\?\&,\+\:@;!\(\)\*\$']*))\]#U", '<a href="$2" class="external">$1</a>', $CON);
	...
	foreach($matches as $m) {
		...
		$CON = str_replace($m[0], '<a href="'.u($m[2]).$attr.'">'.$m[1].'</a>', $CON);
	}
	...
	$CON = preg_replace("/'''''(.*)'''''/Um", '<strong><em>$1</em></strong>', $CON); // bold + italic
